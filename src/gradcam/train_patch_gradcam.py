...
def run_gradcam(
    ...
    """
    Generate and save GradCAM-ready model checkpoint for MIL/patch-based models.

    This function prepares a model checkpoint with all necessary metadata for
    GradCAM visualization on Multi-Instance Learning (MIL) and patch-based models.
    ...
    model : torch.nn.Module
        The MIL/patch-based model to prepare for GradCAM.
    batch_size : int
        Batch size for data loading.
    output : str
        Directory to save the GradCAM checkpoint.
    config_path : str, optional
        Path to configuration file. Default is "config/config.yaml".
    num_patches : int, optional
        Number of patches used in the MIL model.
    arch_type : str, optional
        Architecture type (e.g., 'mil', 'mil_v2', 'patch_resnet').
    ...
    Returns
    -------
    gradcam_model_path : str
        Path to the saved GradCAM checkpoint file.

    Notes
    -----
    - The checkpoint includes: model, input_size, gradcam_layer, model_name,
      normalize, inference_time, num_patches, arch_type
    - GradCAM layer is determined automatically based on architecture type
    - Inference time is measured using a dummy input
    """
    import time
    ...
    if pretrained_model_path:
        ...
        gradcam_model_path = os.path.join(
            output, f"gradcam_{arch_type}_{type(model).__name__}_full.pth"
        )
    
    # Use get_gradcam_layer_patch to determine the correct gradcam layer for patch/MIL models
    ...
    if num_patches is not None:
        ...
        dummy_input = torch.randn(1, num_patches, 3, img_size[0], img_size[1]).to(device)
    else:
    ...
    # Save dict with model, input_size, gradcam_layer, model_name, normalize, inference_time, num_patches, arch_type
    model_info = {
        "model": model,
        "input_size": img_size,
